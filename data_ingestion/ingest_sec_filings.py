...
from backend.app.schemas.models import Stock, SecFiling, Base
...
class SecFilingIngester:
    ...
    def __init__(self):
        ...
        self.headers = {
            'User-Agent': 'Shadow-SEC/1.0 (opensource-financial-watchdog)'
        }
    ...
    async def _fetch_recent_filings(self, symbol: str, days: int) -> List[Dict[str, Any]]:
    ...
    async def _process_and_save_filing(self, stock_id: int, filing_data: Dict[str, Any]):
        """Process and save a SEC filing to the database"""
        try:
            # Check if filing already exists
            existing_filing = self.db.query(SecFiling).filter(
                SecFiling.accession_number == filing_data["accession_number"]
            ).first()
            ...
            text_content = await self._extract_filing_text(filing_data.get("document_url"))
            
            # Process and structure the filing data
            processed_data = await self._process_filing_content(filing_data, text_content)
            ...
            logger.error(f"Failed to process filing {filing_data.get('accession_number', 'unknown')}: {e}")
            self.db.rollback()
    ...
    async def _extract_filing_text(self, document_url: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to fetch filing content: {response.status_code}")
                return ""
    ...
    async def _process_filing_content(self, filing_data: Dict[str, Any], text_content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if text_content:
                ...
                risk_keywords = ["risk", "uncertainty", "competition", "regulatory", "market volatility"]
                if any(keyword in text_lower for keyword in risk_keywords):
                    processed_data["risk_factors_mentioned"] = True
                
                # Check for financial data
                financial_keywords = ["revenue", "earnings", "profit", "cash flow", "balance sheet"]
