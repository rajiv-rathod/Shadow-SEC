...
from backend.app.schemas.models import Stock, MarketData, Base
...
class MarketDataIngester:
    ...
    async def _get_stock_info(self, symbol: str) -> Dict[str, Any]:
        ...
        if not self.enabled:
            return {"name": f"{symbol} Company", "sector": "Technology", "industry": "Software"}
        ...
        try:
            ...
            return {
                ...
                "market_cap": profile.get('market_capitalization', 0) * 1000000  # Convert to actual value
            }
        except Exception as e:
        ...
        try:
            ...
            for stock in stocks:
                try:
                    ...
                    logger.info(f"Ingested {len(historical_data)} historical data points for {stock.symbol}")
                    
                    # Add delay to respect rate limits
                    await asyncio.sleep(1)
                    
                except Exception as e:
                    logger.error(f"Failed to ingest historical data for {stock.symbol}: {e}")
                    continue
    ...
    async def _fetch_historical_data(self, symbol: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    ...
    def _generate_mock_historical_data(self, symbol: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
