...
async def generate_report(
    ...
    try:
        ...
        stock = (
            db.query(StockModel)
            .filter(StockModel.symbol == request.stock_symbol.upper())
            .first()
        )
        if not stock:
            raise HTTPException(
        ...
        logger.info(
            f"Started report generation job {job_id} for stock {request.stock_symbol}"
        )
...
async def get_report_status(job_id: str, db: Session = Depends(get_db)):
    ...
    if job["status"] == "completed" and "report_id" in job:
        result = (
            db.query(ReportModel).filter(ReportModel.id == job["report_id"]).first()
        )
...
async def list_reports(
    ...
    reports = (
        query.order_by(ReportModel.generated_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
async def process_report_generation(
    job_id: str, stock_id: int, request: ReportRequest, db: Session
):
    """
    ...
    try:
        ...
        report_data = ReportCreate(
            ...
            title=ai_response.get(
                "title", f"Correlation Analysis for {request.stock_symbol}"
            ),
