...
from backend.app.schemas.models import (
    Report as ReportModel, Stock as StockModel, 
    MarketData, SecFiling, NewsArticle
)
...
class ReportService:
    ...
    async def gather_analysis_data(
        ...
        try:
            ...
            if not include_sources or "market_data" in include_sources:
                data["market_data"] = await self._gather_market_data(stock_id, start_date, end_date)
            
            # Gather SEC filings
            if not include_sources or "sec_filing" in include_sources:
                data["sec_filings"] = await self._gather_sec_filings(stock_id, start_date, end_date)
            
            # Gather news data
            if not include_sources or "news" in include_sources:
                data["news"] = await self._gather_news_data(stock.symbol, start_date, end_date)
            
            logger.info(f"Gathered analysis data for {stock.symbol}: "
    ...
    async def _gather_market_data(self, stock_id: int, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
    ...
    async def _gather_sec_filings(self, stock_id: int, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        """Gather SEC filings for the specified period"""
        try:
            sec_filings = self.db.query(SecFiling).filter(
            ...
            return [
                {
                    ...
                    "text_content": filing.text_content[:2000] if filing.text_content else None,  # Limit content
    ...
    async def _gather_news_data(self, stock_symbol: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {
                    "title": article.title,
                    "content": article.content[:1000] if article.content else None,  # Limit content
                    "url": article.url,
    ...
    async def get_recent_reports(self, stock_id: Optional[int] = None, limit: int = 10) -> List[ReportModel]:
